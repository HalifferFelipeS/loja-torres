...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function validateOrder() {
        ...
        inputs.forEach(input => {
            ...
            if(values[val]) { values[val]++; hasDuplicates = true; } 
            else { values[val] = 1; }
        });

        inputs.forEach(input => {
            if(values[input.value] > 1) { input.classList.add('input-error'); } 
        ...
        return new Promise((resolve, reject) => {
            ...
            reader.onload = (event) => {
                ...
                img.onload = () => {
                    ...
                    // --- MUDANÇA CRUCIAL: DETETA SE É PNG ---
                    // Se o arquivo original for PNG, mantém como PNG para não perder a transparência.
                    // Caso contrário (JPG, etc), comprime como JPEG.
